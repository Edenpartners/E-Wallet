...
export class EtherApiService {
  ...
  getEthBalance(walletInfo: WalletInfo): Promise < any > {
    return new Promise((finalResolve, finalReject) => {
      const p: EthProviders.Base = this.eths.getProvider(walletInfo.provider);
      const w: Wallet = this.walletService.walletInstance(walletInfo, p.getEthersJSProvider());
  ...
  sendEth(walletInfo: WalletInfo, sendEthTo: string, sendWeiAmount: BigNumber,
    timeout = -1,
    onTransactionCreate: ((any) => void) = (tx) => {},
    onTransactionReceipt: ((any) => void) = null): Promise < any > {
    return new Promise((finalResolve, finalReject) => {
      ...
      const p: EthProviders.Base = this.eths.getProvider(walletInfo.provider);
      const w: Wallet = new ethers.Wallet(walletInfo.info.privateKey, p.getEthersJSProvider());
      ...
      sendPromise.then(
        (tx) => {
          ...
          this.trackTransactionReceipt(p, tx.hash).then((txReceipt) => {
  ...
  getERC20TokenInfo(walletInfo: WalletInfo, ctInfo: ContractInfo, timeout = -1): Promise <
    any > {
  ...
  async getERC20TokenBalance(walletInfo: WalletInfo, ctInfo: ContractInfo, timeout = -1): Promise <
    any > {
      return new Promise < any > (async (finalResolve, finalReject) => {
        if (ctInfo.type !== ContractType.ERC20) {
          finalReject(new Error('this is not an ERC-20 Token'));
  ...
  async transferERC20Token(
    walletInfo: WalletInfo, ctInfo: ContractInfo,
    toAddress: string, sendingAmount: string, timeout = -1,
    onTransactionCreate: ((any) => void) = (tx) => {},
    onTransactionReceipt: ((any) => void) = null
  ): Promise < any > {
    ...
    return new Promise(async (finalResolve, finalReject) => {
      ...
      try {
        adjustedAmount = ethers.utils.bigNumberify(sendingAmount);
        adjustedAmount = adjustedAmount.mul(ethers.utils.bigNumberify(10).pow(ctInfo.contractInfo
          .decimal));
      } catch (e) {
        adjustedAmount = null;
        finalReject(e);
      ...
      // this.abiStorage.etherERC20
      // last argument as provider = readonly but wallet = r/w
      const contract = new Contract(ctInfo.address, this.etherData.abiResolver.getERC20(p),
        w);
      ...
      const estimatedGasBn = await contract.estimate.transfer(toAddress, adjustedAmount);
      console.log('got estimated val : ' + estimatedGasBn + ', gasPrice : ' + gasPrice);
  ...
  trackTransactionReceipt(p: EthProviders.Base,
    txHash: string, repeatIfError: boolean = true, timeout = -1): Promise < any > {
    return new Promise(async (finalResolve, finalReject) => {
      const rp = p.getEthersJSProvider();

      let isComplete = false;
      const worker = () => {
        rp.getTransactionReceipt(txHash).then((txReceipt) => {
  ...
  async kyberNetworkTradeEthToErc20Token(
    ...
    tradeTargetAddress: string, srcEthAmount: string,
    onTransactionCreate: ((any) => void) = (tx) => {},
    onTransactionReceipt: ((any) => void) = null
  ) {
    return new Promise(async (finalResolve, finalReject) => {
      ...
      const ethWeiAmount: BigNumber = ethers.utils.parseEther(String(srcEthAmount));
      ...
      this.kyberNetworkGetExpectedTradeRate(contract,
          this.etherData.contractResolver.getETH(p),
          targetErc20ContractAddres, ethWeiAmount)
        .then((getRateData) => {
          return this.kyberNetworkExecuteTrade(contract, gasPrice,
            targetErc20ContractAddres,
            ethWeiAmount, getRateData.slippageRate);
        }).then((tradeTxData) => {
          onTransactionCreate(tradeTxData);

          if (onTransactionReceipt !== null) {
            this.trackTransactionReceipt(p, tradeTxData.hash).then((txReceipt) => {
              onTransactionReceipt(txReceipt);
            }, (error) => {
              this.logger.debug('txReceiptError');
              this.logger.debug(error);
  ...
  async kyberNetworkExecuteTrade(contract: Contract, gasPrice: BigNumber, destTokenAddress: string,
    srcAmount: BigNumber, minConversionRate: BigNumber): Promise < any > {
    return new Promise(async (finalResolve, finalReject) => {
      const estimatedGasBn = await contract.estimate.swapEtherToToken(
        destTokenAddress,
        minConversionRate, { value: srcAmount });
      console.log('got estimated val : ' + estimatedGasBn);

      const tradePromise = contract.functions.swapEtherToToken(
        destTokenAddress,
        minConversionRate, { value: srcAmount, gasPrice: gasPrice, gasLimit: estimatedGasBn }
      );

      this.logger.debug('run Trade');
      tradePromise.then((tx) => {
          finalResolve(tx);
        },
        (tradeError) => {
  ...
  kyberNetworkGetExpectedTradeRate(contract: Contract, srcTokenAddress: string, destTokenAddress:
    string, srcQty: BigNumber): Promise < any > {
    return new Promise((finalResolve, finalReject) => {
      // Expecting Rate
      ...
      expectedRateResult.then((result: Array < any > ) => {
          ...
          this.logger.debug('expected rate : ' + expectedRate + ', slippage rate : ' +
            slippageRate);
          // run trade
  ...
  kyberNetworkHandleExecuteTradeEvent(contract: Contract, timeout = -1): Promise < object > {
    const executeTradeEvent = new Promise((resolve, reject) => {
      ...
      const transferListner = (trader, srcTokenContractAddress, dstTokenContractAddress,
        actualSrcAmount, actualDestAmount, result) => {
